...
def main(args):
    ...
    if dist > 0.8:
        print("%s and %s are the same person, similarity: %f, average prediction time: %dms" % (
            args.audio1_path, args.audio2_path, dist, round((end - start) * 1000) / 2))
    else:
        print("%s and %s are not the same person, similarity is only: %f, average prediction time: %dms" % (
            args.audio1_path, args.audio2_path, dist, round((end - start) * 1000) / 2))
